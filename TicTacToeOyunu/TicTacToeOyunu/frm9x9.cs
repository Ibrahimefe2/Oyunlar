...
{
    public partial class frm9x9 : Form
    {
        ...
        private void frm9x9_Load(object sender, EventArgs e)
        {
            int buttonSize = 40; // Butonların boyutu (9x9 için daha küçük butonlar)
            int gridSize = 9;    // 9x9 için ızgara boyutu
            ...
            // 9x9 butonları dinamik olarak oluşturuyoruz
        ...
        private bool CheckRowCol(string[,] board)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 6; j++) // Her satırda ve sütunda, 0'dan 5'e kadar kontrol ediyoruz (4'lü kombinasyon)
                {
                    // Satır kontrolü (4 aynı sembol yan yana mı?)
                    if (board[i, j] == board[i, j + 1] && board[i, j + 1] == board[i, j + 2] && board[i, j + 2] == board[i, j + 3] && !string.IsNullOrEmpty(board[i, j]))
                        return true;

                    // Sütun kontrolü (4 aynı sembol üst üste mi?)
                    if (board[j, i] == board[j + 1, i] && board[j + 1, i] == board[j + 2, i] && board[j + 2, i] == board[j + 3, i] && !string.IsNullOrEmpty(board[j, i]))
                        return true;
                }
            }
        ...
        {
            // Sol üstten sağ alta ve sağ üstten sol alta 4'lü kontrol
            for (int i = 0; i < 6; i++)
            {
                for (int j = 0; j < 6; j++)
                {
                    // Sol üstten sağ alta (diagonal) kontrol (4 aynı sembol)
                    if (board[i, j] == board[i + 1, j + 1] && board[i + 1, j + 1] == board[i + 2, j + 2] && board[i + 2, j + 2] == board[i + 3, j + 3] && !string.IsNullOrEmpty(board[i, j]))
                        return true;

                    // Sağ üstten sol alta (diagonal) kontrol (4 aynı sembol)
                    if (board[i, j + 3] == board[i + 1, j + 2] && board[i + 1, j + 2] == board[i + 2, j + 1] && board[i + 2, j + 1] == board[i + 3, j] && !string.IsNullOrEmpty(board[i, j + 3]))
                        return true;
                }
            }
        ...
        {
            // Menüye dönüş işlemi, burada diğer forma geçiş yapabilirsiniz
            frmAnaEkran anaEkran = new frmAnaEkran();
            this.Hide(); // Formu kapatıyor
            anaEkran.Show();
        }
    }
}
