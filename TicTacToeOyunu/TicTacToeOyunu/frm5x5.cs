...
namespace TicTacToeOyunu
{
    public partial class frm5x5 : Form
    {
        ...
        private void frm5x5_Load(object sender, EventArgs e)
        {
            ...
            int gridSize = 5;    // 5x5 için ızgara boyutu
            ...
            // 5x5 butonları dinamik olarak oluşturuyoruz
        ...
        private void CheckForWinner()
        {
            ...
            // Eğer tüm bloklar doluysa ve kazanan yoksa, beraberlik kontrolü
            bool isDraw = true;
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 5; j++)
        ...
        {
            // Satır ve sütun kontrolü için 4'lü kontrol
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 2; j++) // 5'te 4'lü kombinasyonları kontrol ediyoruz
                {
        ...
        {
            // Çapraz kontrol için 4'lü kontrol
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    // Sol üstten sağ alta doğru diagonal kontrol (4 aynı sembol)
                    if (board[i, j] == board[i + 1, j + 1] && board[i + 1, j + 1] == board[i + 2, j + 2] && board[i + 2, j + 2] == board[i + 3, j + 3] && !string.IsNullOrEmpty(board[i, j]))
                        return true;

                    // Sağ üstten sol alta doğru diagonal kontrol (4 aynı sembol)
        ...
        {
            // Menüye dön butonuna basıldığında, ana ekrana geçiş yapıyoruz
            frmAnaEkran anaEkran = new frmAnaEkran();
            this.Hide(); // Şu anki formu gizliyoruz
            anaEkran.Show(); // Ana ekranı gösteriyoruz
        }
    }
}
